...
static func save_settings(settings: Dictionary = Data.settings):
	var file := ConfigFile.new()
	var error: Error = file.load(_SETTINGS_FILE_PATH)
	assert(error == OK)
...
## Creates empty settings file of class ConfigFile with Data.DEFAULT_SETTINGS. Prints error if occured.
static func _create_default_settings_file(file: ConfigFile):
	_write_settings_to_file(Data.DEFAULT_SETTINGS, file)
